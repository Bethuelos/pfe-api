...
{
    // Building Permits registration
    public function register(Request $request, $no_request)
    {
        DB::table('buildingpermit')->insert([
            ...
            "is_certificate_owernership_r" => $request->input('is_certificate_owernership_r'),
            "is_descriptivequote_r" => $request->input('is_descriptivequote_r'),
            "is_groundplan_r" => $request->input('is_groundplan_r'),
            "is_sitesituationplan_r" => $request->input('is_sitesituationplan_r'),
            ...
            "email_f" => $request->input('phone_number_f'),
    ...
    {
        $this->user = Auth::user();
        $PC = DB::table('buildingpermit')
                ->join('request', 'buildingpermit.request_no_request', '=', 'request.no_request')->where('request.user_id_user', '=', $this->user->id_user)
                ->select('buildingpermit.*', 'request.*')
                ->get();
    ...
    // Specific Building Permit info
    public function retrieveSpecific($id)
    {
        $this->user = Auth::user();
        $result = DB::table('buildingpermit')->where('id_buildingpermit', $id)->first();
        if ($result == NULL){
            return response()->json([
                'message' => 'doesn\'t exists'
            ]);
        ...
        $PC = DB::table('buildingpermit')
                ->join('request', 'buildingpermit.request_no_request', '=', 'request.no_request')
    ...
    public function modify(Request $request, $id)
    {
        ...
        $PC = DB::table('buildingpermit')
                ...
                ->first();
        if ($PC != Null){
            DB::table('buildingpermit')
              ->where('id_buildingpermit', $id)
              ->update([
                "last_name" => $request->input('last_name'),
                "fist_name" => $request->input('fist_name'),
                "address" => $request->input('address'),
    ...
    public function delete($id)
    {
        ...
        $PC = DB::table('buildingpermit')
                ...
                ->first();
        if ($PC != Null){
            DB::table('buildingpermit')->where('id_buildingpermit', $id)->delete();
            return response()->json([
